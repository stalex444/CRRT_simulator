...
const Phys = {
  ...
  calcTMP: (base, age, bfr, hct, ac, pfr, ff) => {
    const hf = 1 + Math.max(0, hct - 30) * 0.03;
    const fff = ff > 25 ? 1 + (ff - 25) * 0.02 : 1;
    return base * Math.exp(0.0008 * hf * (2 - ac) * fff * age * 60);
  },
...
// Components
const Gauge = ({ l, v, mn, mx, wl, wh, dl, dh }) => {
  const p = Math.max(0, Math.min(100, ((v - mn) / (mx - mn)) * 100));
  const c = (v <= dl || v >= dh) ? '#ff4444' : (v <= wl || v >= wh) ? '#ffcc00' : '#00ff88';
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px', background: '#0a1525', borderRadius: '4px', border: `1px solid ${c}44` }}>
      <span style={{ fontSize: '8px', color: '#bbccdd', fontWeight: '600' }}>{l}</span>
      <svg width="44" height="44" viewBox="0 0 44 44">
        <circle cx="22" cy="22" r="16" fill="none" stroke="#1a2a3a" strokeWidth="4" strokeDasharray="75 26" strokeDashoffset="12"/>
        <circle cx="22" cy="22" r="16" fill="none" stroke={c} strokeWidth="4" strokeDasharray={`${p * 0.75} ${101 - p * 0.75}`} strokeDashoffset="12" style={{ transition: 'all 0.5s' }}/>
        <text x="22" y="23" textAnchor="middle" fill={c} fontSize="10" fontWeight="700" fontFamily="monospace">{Math.round(v)}</text>
      </svg>
    </div>
  );
};
...
const PatientPanel = ({ pt, calc }) => (
  <div style={{ background: '#0f1525', border: '1px solid #4a6a8a', borderRadius: '5px', padding: '6px' }}>
    <div style={{ fontSize: '10px', fontWeight: '700', color: '#99ccff', marginBottom: '5px' }}>👤 Patient Labs</div>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px', fontSize: '9px' }}>
      {[['K+', pt.k?.toFixed(1), pt.k > 6 ? '#ff6666' : '#66ff99'], ['Hct', pt.hct + '%', '#ff9999'], ['BUN', pt.bun, '#ffcc77'], ['iCa', pt.ca?.toFixed(2), pt.ca < 1 ? '#ff6666' : '#66ff99']].map(([l, v, c]) => (
        <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 4px', background: '#0a1520', borderRadius: '3px', border: '1px solid #2a3a4a' }}>
          <span style={{ color: '#aabbcc', fontWeight: '500' }}>{l}</span><span style={{ color: c, fontFamily: 'monospace', fontWeight: '700' }}>{v}</span>
        </div>
      ))}
    </div>
    {calc && <div style={{ marginTop: '4px', fontSize: '8px', borderTop: '1px solid #3a4a5a', paddingTop: '4px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#99aabb' }}>Clearance:</span><span style={{ color: '#66ddff', fontWeight: '600' }}>{calc.cl?.toFixed(0)} mL/min</span></div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#99aabb' }}>Dose:</span><span style={{ color: '#ffcc77', fontWeight: '600' }}>{calc.dose?.toFixed(1)} mL/kg/h</span></div>
    </div>}
  </div>
);
...
const DrugPanel = ({ eff, fe }) => {
  const [d, setD] = useState('vancomycin');
  const dr = Phys.drugs[d];
  return (
    <div style={{ background: '#1a1a10', border: '1px solid #8a8a5a', borderRadius: '5px', padding: '6px' }}>
      <div style={{ fontSize: '10px', fontWeight: '700', color: '#eedd88', marginBottom: '5px' }}>💊 Drug Dosing</div>
      <select value={d} onChange={e => setD(e.target.value)} style={{ width: '100%', padding: '3px', background: '#1a1a10', border: '1px solid #6a6a4a', borderRadius: '3px', color: '#eedd88', fontSize: '9px', marginBottom: '5px', fontWeight: '500' }}>
        {Object.keys(Phys.drugs).map(k => <option key={k} value={k}>{k.charAt(0).toUpperCase() + k.slice(1)}</option>)}
      </select>
      <div style={{ fontSize: '8px' }}>
        <div style={{ marginBottom: '3px' }}><span style={{ color: '#aaa988' }}>CRRT Dose: </span><span style={{ color: '#66ff99', fontWeight: '600' }}>{dr.c}</span></div>
        <div style={{ marginBottom: '3px' }}><span style={{ color: '#aaa988' }}>Clearance: </span><span style={{ color: '#ffdd66', fontWeight: '600', fontFamily: 'monospace' }}>{dr.cl(eff, fe).toFixed(1)} mL/min</span></div>
        <div style={{ padding: '4px', background: '#12120a', borderRadius: '3px', color: '#bbbb99', fontStyle: 'italic', marginTop: '4px' }}>💡 {dr.note}</div>
      </div>
    </div>
  );
};
...
export default function CRRTSimulator() {
  ...
  useEffect(() => {
    const fe = Phys.filterEff(fs.age, fs.clot, ff);
    setCalc({ cl: Phys.ureaClearance(set.bfr, set.dialysate, set.pfr + set.replacement, fe), dose: eff / pt.wt, fe });
  }, [set, pt, fs, ff, eff]);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      setPr(p => {
        const ac = set.pbp > 1500 ? 0.8 : 0.6;
        return { access: Phys.calcAccess(set.bfr), return: Phys.calcReturn(set.bfr, fs.clot), tmp: Math.min(450, Phys.calcTMP(100, fs.age, set.bfr, pt.hct, ac, set.pfr, ff)), deltaP: Phys.calcDeltaP(set.bfr, fs.clot, pt.hct) };
      });
      if (time % 300 < spd) setPt(p => ({ ...p, k: Math.max(3.5, p.k - 0.03), bun: Math.max(10, p.bun - 0.5) }));
    }, 1000);
    ...
    <div style={{ width: '100%', minHeight: '100vh', background: 'linear-gradient(135deg, #080f1a 0%, #0c1525 100%)', fontFamily: 'system-ui', color: '#e0e8f0', padding: '6px', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '130px 1fr 170px', gap: '6px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
          ...
          <PatientPanel pt={pt} calc={calc} />
          <DrugPanel eff={eff} fe={calc.fe || 1} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', background: '#0a1015', borderRadius: '5px', padding: '3px', border: '1px solid #1a2a3a' }}>
            <Gauge l="Acc" v={pr.access} mn={-350} mx={0} wl={-280} wh={-30} dl={-320} dh={-10} />
            <Gauge l="Ret" v={pr.return} mn={0} mx={400} wl={20} wh={280} dl={10} dh={350} />
            <Gauge l="TMP" v={pr.tmp} mn={0} mx={450} wl={50} wh={280} dl={20} dh={350} />
            <Gauge l="ΔP" v={pr.deltaP} mn={0} mx={200} wl={20} wh={80} dl={10} dh={120} />
          </div>
        </div>
